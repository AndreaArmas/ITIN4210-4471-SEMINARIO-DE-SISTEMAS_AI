...
public class scrDetector : MonoBehaviour
{
    ...
    IEnumerator Upload(byte[] data)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post(" ", form)) //PONER URL AQUÍ
        {
            www.SetRequestHeader("Prediction-Key", " "); //PONER KEY AQUÍ
            ...
            else
            {
                ...
                if (analysisObject.Predictions[0].TagName.Equals("Trumpet"))
                {
                    audio.reproducirTrompeta();
                }
                else if (analysisObject.Predictions[0].TagName.Equals("Piano"))
                {
                    audio.reproducirPiano();
                }
                else if (analysisObject.Predictions[0].TagName.Equals("Electric guitar"))
                {
                    audio.reproducirGuitarra();
                }
